...
if (document && document.location && document.location.pathname && !document.location.pathname.includes('/chrome/newtab')) {
  ...
  window.onmessage = function (msg) {
    if (msg && msg.data && msg.data.action === 'zm:resize' && iframe) {
      clearTimeout(lastTimeout);
      lastTimeout = setTimeout(function () {
        iframe.style.height = "".concat(msg.data.height, "px");
        iframe.style.width = "".concat(msg.data.width, "px");
      }, msg.data.timeout || 10);
    }
  };

  chrome.runtime.onMessage.addListener(function (msg) {
    if (msg && msg.subject === 'widgetStatus') {
      if (msg.payload.active) {
        if (!iframe) {
          iframe = document.createElement('iframe');
          iframe.src = "".concat(msg.payload.extUrl, "widget.html");
